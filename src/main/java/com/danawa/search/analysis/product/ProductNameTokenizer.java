...
public class ProductNameTokenizer extends Tokenizer {
	...
	public boolean incrementToken() throws IOException {
		...
		// 		CharSequence fullString = new CharSequence(stringbuffer, 0, readLengthPrev);
		// 		if (synonymDictionary != null && synonymDictionary.map().containsKey(fullString)) {
		// 			offsetAttribute.setOffset(0, readLengthPrev);
		// 			CharsRef charsRef = termAttribute.charsRef();
		// 			charsRef.chars = stringbuffer;
		// 			charsRef.offset = 0;
		// 			charsRef.length = readLengthPrev;
		// 			// 앞 뒤 공백을 제거한다.
		// 			for (int inx = charsRef.length() - 1; inx >= 0; inx--) {
		// 				if (getType(charsRef.chars[inx]) != WHITESPACE) {
		// 					break;
		// 				}
		// 				charsRef.length--;
		// 			}
		...
		// if (termAttribute.charsRef() != null && termAttribute.charsRef().offset == 0
		// 		&& termAttribute.charsRef().length == 0) {
		// 	return true;
		// }
		...
		// 	token = getChar();
		// 	offsetAttribute.setOffset(baseOffset + token.start(), baseOffset + token.start() + token.length());
		// 	logger.trace("return \"{}\" {}~{}", termAttribute, offsetAttribute.startOffset(),
	...
	private boolean hasToken() throws IOException {
		...
		while (true) {
			...
			// if (position == -1) {
			// 	position = 0;
			// 	termAttribute.setBuffer(buffer, 0, 0);
			// 	return true;
			// }
			...
			// 	termAttribute.setBuffer(buffer, 0, readLength);
			// 	// if(logger.isTraceEnabled()) {
			...
			// 		if (t1 == WHITESPACE) {
			// 			termAttribute.setOffset(positionPrev, position - positionPrev);
			// 			position++;
			// 			return true;
			...
			// 			// 이전버퍼의 남은부분을 리턴하고 끝.
			// 			if (lastLength > 0) {
			// 				termAttribute.setBuffer(buffer, positionPrev, lastLength);
...
// 	private CharSequence getChar() {
// 		CharsRef charsRef = termAttribute.charsRef();
// 		return new CharSequence(charsRef.chars, charsRef.offset, charsRef.length);
// 	}
